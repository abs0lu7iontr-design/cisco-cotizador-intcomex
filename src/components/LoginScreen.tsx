...
export function LoginScreen({ onLoginSuccess }: LoginScreenProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Passwords must match the stored salted hash; plaintext fallbacks are not accepted.
      const computedHash = await sha256Salted(cleanPass);
      const isPasswordValid = Boolean(foundUser.password_hash) && foundUser.password_hash === computedHash;
