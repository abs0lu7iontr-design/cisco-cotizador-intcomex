...
export const ESTIMATES_UPDATED_EVENT = 'cisco-estimates-updated';
...
function notifyEstimatesUpdated() {
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent(ESTIMATES_UPDATED_EVENT));
  }
}
...
/**
 * Subscribes to Firestore estimate changes and falls back to the local cache
 * when Firebase is not configured or unavailable.
 */
export function subscribeToCloudEstimates(
  onChange: (records: CloudEstimateRecord[]) => void,
  onError?: (error: Error) => void,
  limitCount: number = 100
): () => void {
  const { db, isReady } = getFirestoreInstance();
  if (!isReady || !db) {
    onChange(getLocalEstimatesCache());
    return () => undefined;
  }

  const colRef = collection(db, ESTIMATES_COLLECTION);
  const estimatesQuery = query(colRef, orderBy('createdAt', 'desc'), limit(limitCount));

  return onSnapshot(
    estimatesQuery,
    (snapshot) => {
      const records = snapshot.docs.map((docSnap) => ({
        ...(docSnap.data() as Omit<CloudEstimateRecord, 'id'>),
        id: docSnap.id,
      }));
      saveLocalEstimatesCache(records);
      onChange(records);
    },
    (error) => {
      onChange(getLocalEstimatesCache());
...
export const DEFAULT_BASE_USERS: CloudUserRecord[] = [
  {
    ...
    role: 'admin',
    password_hash: 'f1d837d7d6a8b8f28ef38cbad408d5d5f7eb1f0d3a4f7c48ab6b5f68e2d7b0d1',
    is_active: 1,
    ...
    if (legacy) {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        return parsed.map((u: any) => ({
          ...u,
          password_hash: u.password_hash || 'f1d837d7d6a8b8f28ef38cbad408d5d5f7eb1f0d3a4f7c48ab6b5f68e2d7b0d1',
        }));
      }
    }
  } catch (_) {}
...
export async function getCloudUsers(): Promise<{ success: boolean; data: CloudUserRecord[]; error?: string }> {
  try {
    ...
    if (snapshot.empty) {
      ...
      const defaultHash = await sha256Salted('change-me-before-use');
