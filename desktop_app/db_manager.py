...
class DatabaseManager:
    ...
    def _seed_default_users(self):
        """Seeds initial required users with safe placeholders that must be rotated in a real deployment."""
        from auth_manager import AuthManager
        default_password = os.getenv("DEFAULT_DESKTOP_PASSWORD", "change-me-before-use")
        default_pwd_hash = AuthManager.hash_password(default_password)

        users_seed = [
            ("usr-001-admin", "mskill", "System Administrator", "admin@yourcompany.example", default_pwd_hash, "admin"),
            ("usr-002-pm", "madasme", "Product Manager", "pm1@yourcompany.example", default_pwd_hash, "pm"),
            ("usr-003-pm", "rcuevas", "Product Manager", "pm2@yourcompany.example", default_pwd_hash, "pm"),
            ("usr-004-pm", "jvalancia", "Product Manager", "pm3@yourcompany.example", default_pwd_hash, "pm"),
        ]
    ...
    def save_estimate_record(self, estimate_data: Dict[str, Any], items: List[Dict[str, Any]]) -> str:
        """Saves or updates one estimate and its detailed items in the database."""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                SELECT id FROM estimates
                WHERE user_id = ? AND estimate_id_cisco = ? AND deal_id = ? AND original_filename = ?
            ...
            est_id = existing["id"] if existing else f"est-{uuid.uuid4().hex[:8]}"

            if existing:
                cursor.execute("DELETE FROM estimate_items WHERE estimate_id = ?", (est_id,))
                cursor.execute("""
                    UPDATE estimates SET
                        partner_name = ?, client_final_name = ?, stored_filepath = ?,
                        net_cisco_total = ?, total_cotizado_intcomex = ?,
                        recargo_reglas_usd = ?, ganancia_intcomex_usd = ?,
